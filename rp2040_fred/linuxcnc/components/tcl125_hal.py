...
JOG_DEADBAND_MM = env_float("TCL125_JOG_DEADBAND_MM", 0.001)
JOG_MAX_DELTA_MM = env_float("TCL125_JOG_MAX_DELTA_MM", 1.0)
JOG_FEED = env_int("TCL125_JOG_FEED", 100)
SETTLE_TOLERANCE_MM = env_float("TCL125_SETTLE_TOLERANCE_MM", 0.03)
SETTLE_TIMEOUT_S = env_float("TCL125_SETTLE_TIMEOUT_S", 10.0)
TRACE_PATH = env_str("TCL125_TRACE_PATH", "/tmp/tcl125_hal_trace.csv")
...
def target_delta(
    ...
    dx = target[0] - feedback[0]
    dz = target[1] - feedback[1]
...
if TRACE_PATH:
    try:
        ...
        print(f"tcl125: writing command trace to {TRACE_PATH}")
    except OSError as exc:
        trace_file = None
        trace_writer = None
        print(f"tcl125: command trace disabled: {exc}")
...
def trace_command(
    ...
    trace_writer.writerow(
        [
            ...
            "" if expected is None else f"{expected[0]:.6f}",
            "" if expected is None else f"{expected[1]:.6f}",
            int(settling),
        ]
    )
...
def connect_if_due(now: float) -> Optional[FredUsbClient]:
    ...
    try:
        ...
        print("tcl125: connected to RP2040 FRED bridge")
        return new_client
        ...
        print(f"tcl125: USB connect failed: {exc}")
        return None
...
def disconnect(reason: object) -> None:
    ...
    print(f"tcl125: USB disconnected: {reason}")
    ...
    while True:
        ...
        if client is not None:
            try:
                ...
                target = (float(h["x-pos-cmd"]), float(h["z-pos-cmd"]))
                if (
                    not command_active
                    ...
                    delta = target_delta(target, feedback)
                    if has_motion(delta):
                        trace_command(
                            loop_start,
                            target,
                            ...
                            command_active,
                            last_status_idle,
                            (feedback[0] + delta[0], feedback[1] + delta[1]),
                        ...
                        if command_active:
                            settle_target = (feedback[0] + delta[0], feedback[1] + delta[1])
                        h["controller-active"] = command_active
                ...
                if not command_active and not controller_error and spindle_state != last_spindle:
                    ...
                    if on:
                        ...
                        print(
                            "tcl125: spindle start queued: "
                            f"direction={direction} rpm={rpm:.1f}"
                        )
                    else:
                        print("tcl125: spindle stop queued")
